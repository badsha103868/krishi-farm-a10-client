...
const AllCrops = () => {
  ...
  // filterData
  const filteredData = data.filter((cropData) =>
    cropData.name.toLowerCase().includes(searchValue.toLocaleLowerCase())
  );

  return (
    <div>
      <div>
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-center text-primary mb-2 mt-5">
          {" "}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 justify-center gap-6  ">
            {filteredData.length > 0 ? (
              filteredData.map((crop) => (
                <AllCrop key={crop._id} crop={crop}></AllCrop>
              ))
            ) : (
              <div className="flex flex-col col-span-full mt-10  justify-center  items-center">
                ...
                <p className="text-center col-span-full text-red-500 text-5xl mb-5">
                  No Result Found
                </p>
              </div>
            )}
